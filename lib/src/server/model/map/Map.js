...
var Map = /** @class */ (function () {
            ...
            if (EXPORTED_LAYER_NAMES_1.EXPORTED_LAYER_NAMES.includes(layer.name)) {
                ...
                for (var y = 0; y < height; y++) {
                    for (var x = 0; x < width; x++) {
                        ...
                        if (layerData !== 0) {
                            var tile = new Tile_1.Tile(layerData, townDataLayer.getByCoordinates(x, y), x, y)
    ...
    Map.prototype.getTownsState = function () {
        return this.towns.map(function (town) {
            return town.export()
        })
    }
        ...
        var countriesInfo = map_json_1.default.layers
            ...
            .reduce(function (acc, layer) {
                layer.objects.forEach(function (obj) {
                    ...
                    var neighboursString = obj.properties.find(function (property) {
                        return property.name === 'neighbours'
                    }).value
                    var neighbours = neighboursString.split('-')
                    acc.push({
                        name: name,
                        localName: localName,
