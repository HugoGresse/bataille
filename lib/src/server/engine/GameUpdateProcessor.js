...
var GameUpdateProcessor = /** @class */ (function () {
    ...
    GameUpdateProcessor.prototype.run = function () {
        ...
        // 1. Update units
        var step1 = Date.now()
        var _c = this.unitsProcessor.updateUnits(this.map, this.playersById),
            updatedUnits = _c.updatedUnits,
            deletedUnits = _c.deletedUnits
        ...
        if (this.lastUpdatedUnits.length) {
            // 3. Updates towns
            var step2 = Date.now()
            var _d = this.unitsProcessor.updateTownsFromUnits(this.map),
                towns = _d.towns,
                deletedUnits_1 = _d.deletedUnits
            ...
            var step3 = Date.now()
            if (towns.length) {
                for (var _i = 0, _e = this.players; _i < _e.length; _i++) {
                    var player = _e[_i]
                    updatePlayerIncome_1.updatePlayerIncome(this.map.getTownsByCountries(), player, this.emitter)
                }
                ;(_b = this.lastChangedTownsStates).push.apply(_b, towns)
            }
