...
var handlePlayerJoin = function (socket) { return function (playerName) {
    if (!lobby) {
        ...
        lobby = new GameLobby_1.GameLobby(socketEmitter_1, futureGameId_1, function (waitingPlayers, sockets) {
            ...
            for (var _i = 0, waitingPlayers_1 = waitingPlayers; _i < waitingPlayers_1.length; _i++) {
                ...
                var player = new Player_1.Player(sockets[waitingPlayer.socketId], pickUnusedColor_1.pickUnusedColor(game.getPlayers()), waitingPlayer.name);
...
var handlePlayerForceStart = function () { return function (shouldForceStart) {
    if (lobby) {
        lobby.handlePlayerForceStart(shouldForceStart);
    }
}; };
var handlePlayerNewUnit = function (socket) { return function (gameId, data) {
...
var handleClearGame = function () { return function (gameId) {
    var game = games[gameId];
    if (!game) {
        return;
    }
...
var handlePlayerUnit = function (socket) { return function (gameId, event) {
    var game = games[gameId];
    if (!game) {
        return;
    }
