...
describe('GameStats', () => {
    ...
    it('counts games per address, once per game and excluding AIs', () => {
        const store = new GameStats(path.join(tempDir, 'stats.ndjson'))
        const home = '82.1.2.3'
        const other = '90.9.9.9'

        // two people behind one address in the same game: that game counts once for it
        store.recordGameStart(
            'g1',
            [
                { name: 'Hugo', isAI: false, ip: home },
                { name: 'Alice', isAI: false, ip: home },
                { name: 'AI-1', isAI: true },
            ],
            at('2025-08-20')
        )
        store.recordGameStart('g2', [{ name: 'Hugo', isAI: false, ip: home }], at('2025-08-21'))
        store.recordGameStart('g3', [{ name: 'Bob', isAI: false, ip: other }], at('2025-08-22'))

        const summary = store.getStats({ from: '2025-08-01', to: '2025-08-31' })
        expect(summary.gamesByIp).toEqual([
            { ip: home, gameCount: 2, playerCount: 2 },
            { ip: other, gameCount: 1, playerCount: 1 },
        ])
    })
    ...
    it('leaves out players recorded before addresses were captured', () => {
        ...
        store.recordGameStart('new', [{ name: 'Hugo', isAI: false, ip: '82.1.2.3' }], at('2025-08-21'))

        const summary = store.getStats({ from: '2025-08-01', to: '2025-08-31' })
        expect(summary.gamesByIp).toEqual([{ ip: '82.1.2.3', gameCount: 1, playerCount: 1 }])
        expect(summary.topPlayers[0]).toEqual({ name: 'Hugo', gameCount: 2 })
    })

    it('keeps addresses out of the range it was not asked about', () => {
        const store = new GameStats(path.join(tempDir, 'stats.ndjson'))
        store.recordGameStart('g1', [{ name: 'Hugo', isAI: false, ip: '82.1.2.3' }], at('2025-07-31'))
        store.recordGameStart('g2', [{ name: 'Hugo', isAI: false, ip: '82.1.2.3' }], at('2025-08-05'))

        expect(store.getStats({ from: '2025-08-01', to: '2025-08-31' }).gamesByIp).toEqual([
            { ip: '82.1.2.3', gameCount: 1, playerCount: 1 },
        ])
    })
    ...
    it('splits games per address by day and by month, newest period first', () => {
        ...
        store.recordGameStart('g1', [{ name: 'Hugo', isAI: false, ip: home }], at('2025-07-30'))
        store.recordGameStart('g2', [{ name: 'Hugo', isAI: false, ip: home }], at('2025-08-20'))
        store.recordGameStart('g3', [{ name: 'Alice', isAI: false, ip: home }], at('2025-08-20', 18))
        store.recordGameStart('g4', [{ name: 'Bob', isAI: false, ip: other }], at('2025-08-21'))

        const summary = store.getStats({ from: '2025-07-01', to: '2025-08-31' })

        expect(summary.gamesByIpByDay).toEqual([
            { period: '2025-08-21', gameCount: 1, ips: [{ ip: other, gameCount: 1, playerCount: 1 }] },
            { period: '2025-08-20', gameCount: 2, ips: [{ ip: home, gameCount: 2, playerCount: 2 }] },
            { period: '2025-07-30', gameCount: 1, ips: [{ ip: home, gameCount: 1, playerCount: 1 }] },
        ])
        expect(summary.gamesByIpByMonth).toEqual([
            ...
            { period: '2025-07', gameCount: 1, ips: [{ ip: home, gameCount: 1, playerCount: 1 }] },
        ])
        // the range total still spans both months
        expect(summary.gamesByIp).toEqual([
            { ip: home, gameCount: 3, playerCount: 2 },
            { ip: other, gameCount: 1, playerCount: 1 },
        ])
    })
    ...
    it('counts a game shared by one address once within its period', () => {
        ...
        store.recordGameStart(
            'g1',
            [
                { name: 'Hugo', isAI: false, ip: '82.1.2.3' },
                { name: 'Alice', isAI: false, ip: '82.1.2.3' },
            ],
            at('2025-08-20')
        )
        ...
        const summary = store.getStats({ from: '2025-08-01', to: '2025-08-31' })
        expect(summary.gamesByIpByDay).toEqual([
            { period: '2025-08-20', gameCount: 1, ips: [{ ip: '82.1.2.3', gameCount: 1, playerCount: 2 }] },
        ])
    })
